...
function apply(ctx, config = {}) {
	...
	ctx.systemPrompt.section({
		...
		text: ({ scope }) => {
			...
			const lines = [];
			if (has("plan_workflow")) lines.push("Call plan_workflow first when the request is an outcome rather than a defined task: it returns which experts a piece of work needs, in what order, and what each stage must produce. Skip it when the work is one clear job and call the expert directly — planning a two-stage change is overhead, not rigour.");
			lines.push("Each delegation tool below is named for the standing brief it carries. Call one when the work genuinely fits that brief. A child does not see this conversation, so its prompt must stand alone:");
			for (const expert of available) lines.push(`- expert_${expert.slug}: ${expert.description}`);
			lines.push("An expert is a scoping device, not a sandbox — it runs with your privileges and cannot grant you permission you lack. Do not delegate to obtain approval, and do not split work you could finish in one pass.");
			return lines.join("\n");
		}
	});
